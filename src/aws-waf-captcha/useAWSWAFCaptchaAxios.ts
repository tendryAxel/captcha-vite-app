...
import { getWAFEnv } from './util.js'

export default function useAWSWAFCaptchaAxios (onCaptchaEvent = (event) => console.log(event)) {
  ...
  const captchaRequired = (error: undefined) =>
    error.status === 405 && error.headers['x-amzn-waf-action'] === 'captcha'
